<%-- 
    Document   : send
    Created on : Sep 2, 2010, 3:35:24 PM
    Author     : Neo Hu
--%>
...
<%
    ...
    v_mid = ChinabankConfig.mid;
    // 商户自定义返回接收支付结果的页面
    v_url = (String) request.getAttribute("returnUrl");
    // MD5密钥
    key = ChinabankConfig.key;
    // 币种
    v_moneytype = "CNY";
    // 订单金额
    v_amount = String.valueOf(payment.getCNYGatewayAmount());
    ...
    //直连银行编码
    Integer pmode_id = (Integer) request.getAttribute("pmodeId");
...
<form action="https://pay3.chinabank.com.cn/PayGate" method="POST" id="chinabank_submit_form" <c:if test="${not empty requestFromPage && requestFromPage eq 'widget'}">target="_top"</c:if>>
