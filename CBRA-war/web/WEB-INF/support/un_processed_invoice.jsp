<%-- 
    Document   : unProcessedInvoice
    Created on : Sep 18, 2012, 4:24:54 PM
    Author     : Yin.Weilong
--%>
...
<%
    ...
    request.setAttribute("subMenuSelection", "UN_PROCESSED_INVOICE");
%>
<script type="text/javascript">
    function confirmInvoice(subSerialId) {
        if(confirm("请确认发票已经寄出!")) {
            $.getJSON('/support/un_processed_invoice',     
            { a:"CONFIRM_INVOICE",subSerialId:subSerialId},
            function(json) {
                if(json.success) {
                    document.getElementById("msgDiv").innerHTML = "<h5>操作成功</h5>";
                    setTimeout(function(){redirect("/support/un_processed_invoice?start=<fmt:formatDate value='${startDate}'type='date' pattern='yyyy-MM-dd'/>&end=<fmt:formatDate value='${endDate}'type='date' pattern='yyyy-MM-dd'/>");},2000);
                } else {
...
<jsp:include page="/WEB-INF/support/z_date_form.jsp">
    <jsp:param name="action" value="/support/un_processed_invoice" />
    <jsp:param name="title" value="未处理的友付代开发票：" />
</jsp:include>
<div id="msgDiv">
</div>
<c:if test="${not empty orderCollectionSubList}"><div style=""><a href="/support/un_processed_invoice?start=<fmt:formatDate value='${startDate}'type='date' pattern='yyyy-MM-dd'/>&end=<fmt:formatDate value='${endDate}'type='date' pattern='yyyy-MM-dd'/>&a=DOWNLOAD_INVOICE&isProcessed=false">下载CSV文件</a></div></c:if>
        ...
        <c:when test="${not empty orderCollectionSubList}">
            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="history_table">
                ...
                <c:forEach var="order" items="${orderCollectionSubList}" varStatus="status">
                    ...
                    <td><input type="button" onclick="confirmInvoice('${order.subSerialId}');" value="处理" /></td>
                </c:forEach>
                </body>
            </table>
            <div class="vPagging">
