        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                <c:forEach var="plate" items="${plateList}">
                            ,{id:'${plate.id}', pId:'<c:if test="${plate.parentPlate == null}">0</c:if><c:if test="${plate.parentPlate != null}">${plate.parentPlate.id}</c:if>', name:"${plate.name}", open:true, <c:if test="${plate.parentPlate != null}">url:"/admin/plate/plate_auth_info?plateId=${plate.id}",</c:if> target: "listFrame"}
                </c:forEach>
                ];
                $(document).ready(function () {
