...
<script type="text/javascript">
	$(function() {
  		...
  		$("#saveBtn").click(function() {
			var rules={
				"district.name": {required: true}
			};
			var messages={
				"district.name": {required: "区域名称必须填写！"}
			};
			//初始化验证框架
			FormSave("form1",rules, messages);
			$("#form1").attr("target", "iframe1");
			$("#form1").attr("action", "districtAction!saveDistrict.action");
            $("#form1").submit();
        });
  		$.fn.goback();
  	});
...
<body>
	...
	<s:form id="form1" name="form1" method="post" theme="simple">
		<s:hidden id="districtId" name="district.id"></s:hidden>
		<s:hidden id="sortNum" name="district.sortNum"></s:hidden>
		<s:hidden id="cityId" name="cityId"></s:hidden>
	    <ul class="forminfo">
	    <li><label>区域名称<b>*</b></label><s:textfield id="txt_districtName" name="district.name" cssClass="dfinput" cssStyle="width: 350px;" maxlength="25"></s:textfield><i>区域名称不能超过25个汉字，必填项</i></li>
	    <li><label>区域编码</label><s:textfield id="code" name="district.code" cssClass="dfinput" cssStyle="width: 350px;" maxlength="25"></s:textfield><i>区域编码不能超过25个汉字</i></li>
	    <li><label>&nbsp;</label>
